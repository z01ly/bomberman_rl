...
def setup(self):
    if self.train and not os.path.isfile("my-saved-model.pt"):
        ...
        self.model = np.zeros((6,110))
    else:
        self.logger.info("Loading model from saved state.")
        with open("my-saved-model.pt", "rb") as file:
            self.model = pickle.load(file)

    self.init_epsilon = 0.25
...
def state_to_features(last_action, game_state: dict) -> np.array:
    ...
    feat = []
    x,y = game_state['self'][3]
    grid = [(x+1,y),(x+2,y),(x-1,y),(x-2,y),(x,y+1),(x,y+2),(x,y-1),(x,y-2),(x+1,y+1),(x+1,y-1),(x-1,y+1),(x+1,y-1)]
    bomb_pos = [i[0] for i in game_state['bombs']]
    opp_pos = [i[3] for i in game_state['others']]

    for i in grid:
        xc, yc = i
        tile_arr = [0,0,0,0,0,0,0,0]
    ...
    if len(opp_pos) != 0:
        ...
        opp_dir = pos_diff_opp[nearest_opp]
        opp_arr[0] = np.sign(opp_dir[0])*(16 - abs(opp_dir[0]))/8
        opp_arr[1] = np.sign(opp_dir[1])*(16 - abs(opp_dir[1]))/8
    ...
    for bomb in bomb_pos:
        ...
        if xb == x:
            if -4 < yb-y < 0:
                if x%2 == 1:bomb_dir[0] = 10
            if 0 < yb-y < 4:
                if x%2 == 1:bomb_dir [1] = 10
        if yb == y:
            if -4 < xb-x < 0:
                if y%2 == 1:bomb_dir[2] = 10
            if 0 < xb-x < 4:
                if y%2 == 1:bomb_dir [3] = 10
